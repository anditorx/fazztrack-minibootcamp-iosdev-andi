...
class CoffeeShopListViewController: UIViewController, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "coffeeShopCellId", for: indexPath) as! CoffeeShopTableViewCell
        
        cell.thumbImageView.image = UIImage(named: coffeeshopImage[indexPath.row])
        cell.nameLable.text = coffeeshopNames[indexPath.row]
